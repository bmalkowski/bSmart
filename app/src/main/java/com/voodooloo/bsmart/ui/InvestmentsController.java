...
public class InvestmentsController implements Controller {
    ...
    @Inject
    public InvestmentsController(App app, AccountData accountData) {
        this.app = app;
    ...
    void rebuildAccounts() {
        ...
        for (Account account : accountData.findAll()) {
    ...
    void onAdd(ActionEvent event) {
        ...
        if (response == saveAction) {
            accountData.insert(builder.build());
        }
    }

    @Subscribe
    public void onEvent(AccountData.Event event) {
        rebuildAccounts();
    }
}
